...
class TestSeed:
    def test_first_run_inserts_expected_rows(self, db_session):
        ...
        # M5a: 57 triples — 54 M4 triples + 3 new M5a (role_email read/write/delete).
        assert _count(db_session, Permission) == 57, "Expected 57 seeded permission triples at M5a"
        assert _count(db_session, User) >= 10, "Expected at least the 10 seeded users"
        assert _count(db_session, RolePermission) >= 57, "Expected at least 57 role→permission rows"
