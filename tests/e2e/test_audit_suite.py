...
class TestAuditLogDefaultView:
    def test_default_view_shows_recent_entries(self, page: Page) -> None:
        ...
        # At least one column header should be visible (the table renders)
        # TODO: verify selector against rendered DOM during gate ritual
        expect(
            page.get_by_text("When").first
...
class TestAuditLogFilters:
    def test_filter_by_action_apply_button(self, page: Page) -> None:
        ...
        # TODO: verify "Apply" button text matches rendered DOM during gate ritual
...
class TestAuditLogPagination:
    def test_pagination_buttons_render_with_correct_disable_state(
        ...
        prev_btn = page.get_by_role("button", name=re.compile(r"Prev", re.IGNORECASE))
...
class TestAuditLogDetailDrawer:
    def test_open_detail_drawer(self, page: Page) -> None:
        ...
        expect(summary.first).to_be_visible(timeout=10_000)
        # Click the first kebab menu (⋮) to open row actions
        # TODO: verify kebab button selector against rendered DOM during gate ritual
        kebab = page.locator("button:has-text('⋮')").first
        ...
        # TODO: verify "OCCURRED AT" label selector against rendered DOM during gate ritual
        expect(
            page.get_by_text("OCCURRED AT").first
        ).to_be_visible(timeout=5_000)
        # Close the drawer via the ✕ button
...
class TestAuditLogCsvExport:
    def test_csv_export_triggers_download(self, page: Page) -> None:
        ...
        summary = page.locator("text=/Showing \\d/")
        expect(summary.first).to_be_visible(timeout=10_000)
        # Click Export CSV — should trigger download
