"""Unit tests for LetterheadAssetService — upload, replace, deactivate."""
...
class TestUploadLetterhead:
    ...
    def test_replaces_existing(self):
        repo = MagicMock()
        old = MagicMock()
        repo.get_active_by_role_and_scope.return_value = old
    ...
    def test_success_saves_and_returns(self):
        repo = MagicMock()
        repo.get_active_by_role_and_scope.return_value = None
        upload_svc = MagicMock()
        file_asset = MagicMock()
        file_asset.id = uuid4()
        upload_svc.upload.return_value = file_asset
...
class TestSoftDelete:
    def test_not_found_raises(self):
        ...
        svc = _make_svc(repo)
        with pytest.raises(LetterheadError, match="not found"):
