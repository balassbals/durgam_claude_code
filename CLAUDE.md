...
**Current milestone**: M10.5 Phase 2b shipped (`9a53984`, `16dce2a`, `3724cc9`, `848683e`, `1940411`, `0871074` — 6 substantive commits, 60 pages migrated); Phase 4 next (component library expansion). Phase 2a shipped `5985596`. M10 CLOSED (tag m10-close). M10 (Faculty Module) — **CLOSED** 2026-08-20. Gate verified: 61-failure deterministic baseline (fresh-clone diff-empty) + 118 E2E passed + 8 skipped. Tag `m10-close` at `476e334`; merge to main pending. M10 branch: m10-faculty. Phase P1 shipped (`27d323a`) + P1.1 fix (`f2b671b`) + P2 photo upload (`d3ba34d`) + P2.1 (`2aba531`) + P2.2 (`637b7bb`) + P2.3 photo URL fix (`b9ba64f`) + P3a education CRUD (`23dcdaf`) + P3a.1 dialog close fix (`b99fdfe`) + P3a.2 Add/Edit modal close fix (`9613993`) + P3b/P3c experience+expertise CRUD (`d1468cb`) + P4 documents PDF upload (`5ec8b65`) + P4.1 compile fix (`ee54424`) + P6 admin faculty directory (`ad703f8`) + P6.1 fix (`7ca3701`) + Phase 6 FDP quick-access nav (`5f4b1e9`) + Phase 8A faculty directory+detail (`5abedb3`) + 8A.1 grid fix (`e795979`) + Phase 8B faculty requests overlay (`018773d`) + Phase 9A NRF contract-term expansion (`adfb662`) + 9A.1 renew-wiring fix (`d6d94b3`) + 9B closure docs + 10A STEP-A proof (`e2ea313`) + 10B HoD recommend-via live wiring (`8023a57`) + 10B.1 test-count fix (`0c5faf8`) + 11A faculty_id backfill on 3 assignment tables (`c88f4c7`) + 11A.1 test-fixture fix (`f1fe0ad`) + 11B faculty_id backfill on 2 non-owned tables (`813482a`+`2e6ee41`) + 11C Faculty picker endpoint + M5b dropdown rollout (`ba50961`) + 11C.1 test-ordering hardening (`2dd610c`) + 11C.2 fix flaky test_active_only (`0709d16`) + 11D remove class_coordinator_assignments per Q-P11D.1 (`6cf0f54`) + 11D.1 fix 2 test regressions from 11D (`2feee1a`) + 11E confirmation reset on material mentor edit/delete (`fa3893c`) + 11E.1 harden picker helpers 4-hex→8-hex (`fd91d0b`) + 11E.2 suppress re-confirm banner when list empty (`4442d61`) + 12 faculty bulk CSV import (`1c00cfb`) + 12.1 Q-P12.4 correction auto-create User on import (`14571dc`) + Phase 13 TD sweep (`d3c069c` TD-064+TD-069, `b568d94` TD-087, `d2a6323` docs) + Phase 13 SHA-pin (`8ee4d1a`) + Phase 14-pre stale-selector fix (`3957e13`) + Phase 14-pre SHA-pin (`476e334`). Phase P5 (faculty PAN/Aadhaar UI) never shipped — deferred pending TD-084. Full close details + carry-forward TD list: `docs/milestones/M10.md` § M10 Close and `docs/tech_debt.md`. **Next milestone: M11 (Faculty Hardening + Encryption + Test Hygiene)** — scope draft in `docs/milestones/M11.md`; not yet started, branch not yet created.
...
## Current milestone
**M10.5 Phase 2b shipped (`9a53984`, `16dce2a`, `3724cc9`, `848683e`, `1940411`, `0871074` — 6 substantive commits, 60 pages migrated to `app_shell()`); Phase 4 next (component library expansion).** Phase 2a shipped `5985596`. M10 CLOSED (tag m10-close). Phase 1 shipped `89b7a0d`. M10.5 scope, phase table, and Q-decisions: `docs/milestones/M10.5.md`. Branch `m10-5-ui`, forked from `main` at `2eb8b0b`.

**M10 (Faculty Module) — CLOSED 2026-08-20 on branch `m10-faculty`. Gate verified: 61-failure deterministic baseline (fresh-clone diff-empty) + 118 E2E passed + 8 skipped — see `docs/prompts/gate_verification.md` M10 Gate Lessons. Tag `m10-close` at `476e334`; merge to main pending. M9 (Announcement Module) closed 2026-06-14, merged as 2d819c4, tag m9-close on afc1f4b. Post-merge docs commit `7842eb9`. Phase P1 shipped (`27d323a`) + P1.1 fix (`f2b671b`) + P2 photo upload (`d3ba34d`) + P2.1 (`2aba531`) + P2.2 (`637b7bb`) + P2.3 photo URL fix (`b9ba64f`) + P3a education CRUD (`23dcdaf`) + P3a.1 dialog close fix (`b99fdfe`) + P3a.2 Add/Edit modal close fix (`9613993`) + P3b/P3c experience+expertise CRUD (`d1468cb`) + P4 documents PDF upload (`5ec8b65`) + P4.1 compile fix (`ee54424`) + P6 admin faculty directory (`ad703f8`) + P6.1 fix (`7ca3701`) + Phase 6 FDP quick-access nav (`5f4b1e9`) + Phase 8A faculty directory+detail (`5abedb3`) + 8A.1 grid fix (`e795979`) + Phase 8B faculty requests overlay (`018773d`) + Phase 9A NRF contract-term expansion (`adfb662`) + 9A.1 renew-wiring fix (`d6d94b3`) + 9B closure docs + 10A STEP-A proof (`e2ea313`) + 10B HoD recommend-via live wiring (`8023a57`) + 10B.1 test-count fix (`0c5faf8`) + 11A faculty_id backfill on 3 assignment tables (`c88f4c7`) + 11A.1 test-fixture fix (`f1fe0ad`) + 11B faculty_id backfill on 2 non-owned tables (`813482a`+`2e6ee41`) + 11C Faculty picker endpoint + M5b dropdown rollout (`ba50961`) + 11C.1 test-ordering hardening (`2dd610c`) + 11C.2 fix flaky test_active_only (`0709d16`) + 11D remove class_coordinator_assignments per Q-P11D.1 (`6cf0f54`) + 11D.1 fix 2 test regressions from 11D (`2feee1a`) + 11E confirmation reset on material mentor edit/delete (`fa3893c`) + 11E.1 harden picker helpers 4-hex→8-hex (`fd91d0b`) + 11E.2 suppress re-confirm banner when list empty (`4442d61`) + 12 faculty bulk CSV import (`1c00cfb`) + 12.1 Q-P12.4 correction auto-create User on import (`14571dc`) + Phase 13 TD sweep (`d3c069c` TD-064+TD-069, `b568d94` TD-087, `d2a6323` docs) + Phase 13 SHA-pin (`8ee4d1a`) + Phase 14-pre stale-selector fix (`3957e13`) + Phase 14-pre SHA-pin (`476e334`). Phase P5 (faculty PAN/Aadhaar UI) never shipped — deferred pending TD-084. Docs sweep commit `e7fb21d` finalized all M10 documentation before merge to main. Full M10 close details (gate results, tag, phase list, carry-forward TDs): `docs/milestones/M10.md` § M10 Close. Carry-forward tech-debt list: `docs/tech_debt.md` (see TD-084, TD-085, TD-038, TD-039, TD-086, TD-044, TD-089, TD-068, TD-045/046/053/054/056/057/062/066/067, TD-083.1 → M11; TD-088 → M13).**

**Next milestone:** M11 (Faculty Module Hardening + Encryption + Test Hygiene) — scope draft in `docs/milestones/M11.md`. Not yet started; branch not yet created. Pending Bala + stakeholder prioritization of the carry-forward TD list above.
