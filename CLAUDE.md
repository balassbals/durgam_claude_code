...
## Current milestone
**M10 (Faculty Module) — in progress on branch `m10-faculty`. M9 (Announcement Module) closed 2026-06-14, merged as 2d819c4, tag m9-close on afc1f4b. Post-merge docs commit `7842eb9`. Phase P1 shipped (`27d323a`) + P1.1 fix (`f2b671b`) + P2 photo upload (`d3ba34d`) + P2.1 (`2aba531`) + P2.2 (`637b7bb`) + P2.3 photo URL fix (`b9ba64f`) + P3a education CRUD (`23dcdaf`) + P3a.1 dialog close fix (`b99fdfe`) + P3a.2 Add/Edit modal close fix (`9613993`) + P3b/P3c experience+expertise CRUD (`d1468cb`) + P4 documents PDF upload (`5ec8b65`) + P4.1 compile fix (`ee54424`) + P6 admin faculty directory (`ad703f8`) + P6.1 fix (`7ca3701`) + Phase 6 FDP quick-access nav (`5f4b1e9`) + Phase 8A faculty directory+detail (`5abedb3`) + 8A.1 grid fix (`e795979`) + Phase 8B faculty requests overlay (`018773d`) + Phase 9A NRF contract-term expansion (`adfb662`) + 9A.1 renew-wiring fix (`d6d94b3`) + 9B closure docs + 10A STEP-A proof (`e2ea313`) + 10B HoD recommend-via live wiring (`8023a57`) + 10B.1 test-count fix (`0c5faf8`) + 11A faculty_id backfill on 3 assignment tables (`c88f4c7`) + 11A.1 test-fixture fix (`f1fe0ad`) + 11B faculty_id backfill on 2 non-owned tables (`813482a`+`2e6ee41`) + 11C Faculty picker endpoint + M5b dropdown rollout (`ba50961`) + 11C.1 test-ordering hardening (`2dd610c`) + 11C.2 fix flaky test_active_only (`0709d16`) + 11D remove class_coordinator_assignments per Q-P11D.1 (`6cf0f54`) + 11D.1 fix 2 test regressions from 11D (`2feee1a`) + 11E confirmation reset on material mentor edit/delete (`fa3893c`) + 11E.1 harden picker helpers 4-hex→8-hex (`fd91d0b`) + 11E.2 suppress re-confirm banner when list empty (`4442d61`) + 12 faculty bulk CSV import (`1c00cfb`) + 12.1 Q-P12.4 correction auto-create User on import (`14571dc`) + Phase 13 TD sweep (`d3c069c` TD-064+TD-069, `b568d94` TD-087, `d2a6323` docs); Phase P5 deferred pending TD-084; TD-038 + TD-039 + TD-083.1 + TD-085 + TD-086 + TD-088 deferred to M11; TD-084 deferred to M11 as design phase. Phase 13 + 14-pre shipped (`3957e13`); Phase 14 gate ritual in progress.**
