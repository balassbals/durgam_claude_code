...
| ClassTimingsConfig (singleton) | §9.3 | M3 | In flight at M3 | Model + seed; edit page placeholder |
| WorkingDaysConfig (singleton) | §9.3 | M3 | In flight at M3 | Model + seed; edit page placeholder |
...
| Requirement | Source | Milestone | Status | Notes |
|---|---|---|---|---|
| Schools seeded once; departments declare school | §9.3 | M3 | In flight at M3 | Seed script; school_id FK enforced |
...
| Vision/mission: update-only, no delete (university + department) | E-001 | M3 | In flight at M3 | NotDeletableError in VisionMissionService |
| Vision/mission: viewable by all authenticated users | E-001 | M3 | Planned (Session 7) | /about/university + /about/departments pages |
| Class timings and working-days config: singleton, configure action | §9.3, §12 M3 | M3 | In flight at M3 | Model + seed; edit pages → Session 7 |
| "System admin will only deal with basic information of academic departments when adding, editing" | §9.3 | M3 | In flight at M3 | department:write:* granted only to SYSTEM_ADMIN (fixed M3 Session 5c — Registrar had it incorrectly) |
| Dean role bound to school via dean_role_code string reference | §8.2 | M3 | In flight at M3 | OQ-M3-6 confirmed: plain string, no FK |
