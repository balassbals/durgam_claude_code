"""cta_faculty_id_backfill

M10 Phase 11A (Q-P11.4 / D-020) — backfill faculty_id on class_teacher_assignments.
...
Revision ID: d3a9b2c7f184
Revises: c2f8a1b6e733
...
revision: str = "d3a9b2c7f184"
down_revision: Union[str, Sequence[str], None] = "c2f8a1b6e733"
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None

_TBL = "class_teacher_assignments"


def upgrade() -> None:
    op.add_column(_TBL, sa.Column("faculty_id", sa.Uuid(), nullable=True))
    op.create_index("ix_cta_faculty_id", _TBL, ["faculty_id"])
    op.create_foreign_key(
        "fk_cta_faculty_id", _TBL, "faculties", ["faculty_id"], ["id"]
    )
    op.execute(
        f"""
        UPDATE {_TBL} AS a
...
def downgrade() -> None:
    ...
    op.drop_constraint("fk_cta_faculty_id", _TBL, type_="foreignkey")
    op.drop_index("ix_cta_faculty_id", table_name=_TBL)
